...
<div class="center-preview" :style='{"padding":"0 7%","margin":"20px auto 40px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
	...
	<div class="info" :style='{"padding":"10px","boxShadow":"0 0px 0px rgba(0, 0, 0, 0.3)","margin":"20px 0 0 0","borderColor":"#1abc9e30","borderRadius":"0px","background":"none","borderWidth":"0 0 0 0px","width":"25%","borderStyle":"solid","height":"auto","order":"2"}'>
		...
		<div :style='{"borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","width":"90%","fontSize":0,"borderStyle":"solid","height":"auto"}' v-if="userTableName=='jiudianguanliyuan'">
		...
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='jiudianguanliyuan'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}'>管理账号</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.guanlizhanghao}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='jiudianguanliyuan'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}'>管理姓名</div>
			<div :style='{"fontSize":"14px","color":"#999","textAlign":"left","flex":1}'>{{sessionForm.guanlixingming}}</div>
		</div>
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='jiudianguanliyuan'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
		...
		<div :style='{"padding":"0 20px","borderColor":"#1abc9e20","margin":"0 auto","borderWidth":"0 0 1px 0","display":"flex","width":"90%","lineHeight":"40px","borderStyle":"solid","height":"auto"}' v-if="userTableName=='jiudianguanliyuan'">
			<span class="icon iconfont icon-shouye-zhihui" :style='{"padding":"0 5px","fontSize":"14px","color":"#333","display":"none"}'></span>
			<div :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}'>是否审核</div>
    ...
    <el-tabs tab-position="left" :style='{"width":"72%","margin":"20px 0 0","flexWrap":"wrap","background":"#fff","display":"flex","order":"1"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='jiudianguanliyuan'" label="管理账号" prop="guanlizhanghao">
            <el-input v-model="sessionForm.guanlizhanghao" placeholder="管理账号" readonly></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='jiudianguanliyuan'" label="管理姓名" prop="guanlixingming">
            <el-input v-model="sessionForm.guanlixingming" placeholder="管理姓名" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='jiudianguanliyuan'" label="性别" prop="xingbie">
            <el-select v-model="sessionForm.xingbie" placeholder="请选择性别" >
              <el-option v-for="(item, index) in dynamicProp.xingbie" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='jiudianguanliyuan'" label="营业执照" prop="touxiang">
			<file-upload
			tip="点击上传营业执照"
			action="file/upload"
			:limit="1"
			...
			@change="jiudianguanliyuantouxiangHandleAvatarSuccess"
			></file-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"#fff"}' v-if="userTableName=='jiudianguanliyuan'" label="电话号码" prop="dianhuahaoma">
...
<script>
  ...
  export default {
    ...
    created() {
		...
		if ('jiudianguanliyuan' == this.userTableName) {
		...
		if ('jiudianguanliyuan' == this.userTableName&&this.rules['guanlizhanghao']){
			this.rules['guanlizhanghao'].push({ required: true, message: '请输入管理账号', trigger: 'blur' })
		}else if('jiudianguanliyuan' == this.userTableName&&!this.rules['guanlizhanghao']) {
			this.$set(this.rules, 'guanlizhanghao', [{ required: true, message: '请输入管理账号', trigger: 'blur' }]);
		}
		if ('jiudianguanliyuan' == this.userTableName&&this.rules['guanlixingming']){
			this.rules['guanlixingming'].push({ required: true, message: '请输入管理姓名', trigger: 'blur' })
		}else if('jiudianguanliyuan' == this.userTableName&&!this.rules['guanlixingming']) {
			this.$set(this.rules, 'guanlixingming', [{ required: true, message: '请输入管理姓名', trigger: 'blur' }]);
		}
		if ('jiudianguanliyuan' == this.userTableName&&this.rules['mima']){
			this.rules['mima'].push({ required: true, message: '请输入密码', trigger: 'blur' })
		}else if('jiudianguanliyuan' == this.userTableName&&!this.rules['mima']) {
    ...
    methods: {
      ...
      onSubmit(formName) {
		if(`jiudianguanliyuan` == this.userTableName && this.sessionForm.touxiang!=null){
      ...
      jiudianguanliyuantouxiangHandleAvatarSuccess(fileUrls) {
